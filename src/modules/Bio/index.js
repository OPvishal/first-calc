        ...
        <>
          <div id = "MainBody" style = {{display:"flex"}}>
            <div id = "Content" style = {{width:"75%"}}>
              ...
              {
                this.state.q_array.map((el,id)=>
                <div>{"Q"+(id+1).toString()+" "}{el}</div>
                )
              }
                <div id = "Question" style = {this.state.submit?{display:"none"}:{}}>
                  <input value={this.state.temp} onChange = {(c)=>{
                    console.log(c)
                    this.setState({temp:c.target.value})}} ></input>


                  <button onClick = {()=>{this.setState({q_array:[...this.state.q_array,this.state.temp],temp:""})}}>
                    Submit
                  </button>
                </div>
            </div>
